...
public class BaggageHandler : IObservable<BaggageInfo>
{
    ...
    {
        //Check if an observer is already registered. If not, add it.
    ...
    public void BaggageStatus(int flightNumber, string from, int carousel)
    {
        ...
        {
            //Baggage claims for flight is done.
            if (_flights.RemoveWhere(flight =>
                flight.FlightNumber == info.FlightNumber) > 0)
            {
                foreach (var observer in _observers)
                    observer.OnNext(info);
            }
